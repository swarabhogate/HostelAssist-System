...
$welcomeName = isset($_SESSION['name']) ? trim((string) $_SESSION['name']) : 'Warden';
$welcomeShortName = $welcomeName !== '' ? strtok($welcomeName, ' ') : 'Warden';
...
function wardenDashboardBadgeClass($status)
{
    ...
    if (in_array($status, ['assigned', 'in progress', 'pending warden'], true)) {
        return 'badge-info';
    }
...
$complaints = [];
$complaintResult = $conn->query("SELECT c.complaint_id, c.title, c.status, c.completion_date, c.show_name, s.name
                                  FROM complaints c
                                  JOIN students s ON c.student_id = s.student_id
                                  WHERE c.deleted_by_warden = 0
...
$gatepasses = [];
// Warden sees ALL gate pass requests (all statuses) for full visibility and record-keeping.
// Approve/Reject actions only appear on the detail page when status = 'Pending Warden'.
$gatepassResult = $conn->query("SELECT gp.gatepass_id, gp.date_going AS request_date, gp.status, s.name
                                 FROM gate_pass gp
                                 JOIN students s ON gp.student_id = s.student_id
                                 WHERE gp.deleted_by_warden = 0
            ...
            <div class="dashboard-card">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    ...
                    <div class="d-flex align-items-center" style="gap:8px;">
                        <a class="report-btn" href="export_report.php?type=complaints"><i class="fa fa-download"></i> Report</a>
                        <button type="button" class="tool-btn" data-toggle-delete="wardenComplaints"><i class="fa fa-trash"></i></button>
                    </div>
                </div>
                <input type="text" id="wardenComplaintSearch" class="form-control search-bar mb-3" placeholder="Search Complaint ID...">
                <form method="POST" action="bulk_delete_records.php" id="wardenComplaints">
                    <input type="hidden" name="record_type" value="complaint">
                    <button type="submit" class="delete-submit" onclick="return confirm('Delete selected complaints?');"><i class="fa fa-trash"></i> Delete</button>
                    <div class="list-container">
                        <?php if (!empty($complaints)) { foreach ($complaints as $row) {
                            $badgeClass = wardenDashboardBadgeClass($row['status']);
                            $completionDateText = '';
                            if (in_array(strtolower(trim((string) $row['status'])), ['resolved'], true) && !empty($row['completion_date'])) {
                                $completionDateText = '<br><small>Completion Date: ' . htmlspecialchars(date('d/m/Y', strtotime($row['completion_date']))) . '</small>';
                            }
                            $displayName = strtolower(trim((string) ($row['show_name'] ?? 'yes'))) === 'no' ? 'Anonymous' : htmlspecialchars((string) $row['name']);
                            $canDelete = strtolower(trim((string) $row['status'])) === 'resolved';
                            echo '<div class="list-row" data-search="#'.$row['complaint_id'].' '.$row['title'].' '.$displayName.'">';
                            if ($canDelete) {
                                echo '<input class="delete-checkbox" type="checkbox" name="selected_ids[]" value="'.(int) $row['complaint_id'].'">';
                            } else {
                                echo '<span class="delete-checkbox" style="visibility:hidden;"></span>';
                            }
                            echo '<a href="complaint_info.php?id='.(int) $row['complaint_id'].'" class="item-link"><div class="list-item"><div><strong>#'.(int) $row['complaint_id'].'</strong> - '.htmlspecialchars((string) $row['title']).$completionDateText.'<br><small>'.$displayName.'</small></div><span class="badge '.$badgeClass.'">'.htmlspecialchars((string) $row['status']).'</span></div></a>';
                            echo '</div>';
            ...
            <div class="dashboard-card">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    ...
                    <div class="d-flex align-items-center" style="gap:8px;">
                        ...
                        <button type="button" class="tool-btn" data-toggle-delete="wardenGatepasses"><i class="fa fa-trash"></i></button>
                    </div>
                </div>
                <input type="text" id="wardenGatepassSearch" class="form-control search-bar mb-3" placeholder="Search Gatepass ID...">
                <form method="POST" action="bulk_delete_records.php" id="wardenGatepasses">
                    <input type="hidden" name="record_type" value="gatepass">
                    <button type="submit" class="delete-submit" onclick="return confirm('Delete selected gatepasses?');"><i class="fa fa-trash"></i> Delete</button>
                    <div class="list-container">
                        <?php if (!empty($gatepasses)) { foreach ($gatepasses as $entry) {
                            $badgeClass = wardenDashboardBadgeClass($entry['status']);
                            ...
                            echo '<a href="view_gatepass.php?id='.(int) $entry['gatepass_id'].'" class="item-link"><div class="list-item"><div><strong>GP#'.(int) $entry['gatepass_id'].'</strong> - '.htmlspecialchars((string) $entry['request_date']).'<br><small>'.htmlspecialchars((string) $entry['name']).'</small></div><span class="badge '.$badgeClass.'">'.htmlspecialchars((string) $entry['status']).'</span></div></a>';
                            echo '</div>';
...
setupSearch('wardenComplaintSearch', 'wardenComplaints');
setupSearch('wardenGatepassSearch', 'wardenGatepasses');
</script>
</body>
</html>
